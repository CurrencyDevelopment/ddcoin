...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #DDCoinTEST2\r");
            Send(hSocket, "WHO #DDCoinTEST2\r");
        } else {
            // randomly join #DDCoin00-#DDCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #DDCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #DDCoin%02d\r", channel_number).c_str());
        }
